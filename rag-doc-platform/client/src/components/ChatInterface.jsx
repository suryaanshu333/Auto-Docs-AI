                      ...
                      {msg.sources && msg.sources.length > 0 && (
                        <div className="mt-3 flex flex-wrap gap-2">
                          {msg.sources.map((s, si) => {
                            const sourceName = (s.metadata && s.metadata.source) || `Source ${si + 1}`;
                            const snippet = (s.chunk || '').slice(0, 140).replace(/\n/g, ' ');
                            return (
                              <button
                                key={si}
                                onClick={() => {
                                  try {
                                    console.log('source button clicked', { hasOpenPreview: !!openPreview, sourceIndex: si, sourcePreview: (s.chunk || '').slice(0, 120) });
                                  } catch (e) { }
                                  openPreview && openPreview(s.chunk);
                                }}
                                className="rounded-md border border-zinc-700 px-3 py-1 text-xs text-zinc-300 hover:border-zinc-500 hover:bg-[#1b1b1b]"
                                title={`Jump to source: ${sourceName}`}
                              >
                                <strong className="mr-2 text-zinc-200">{sourceName}</strong>
                                <span className="text-zinc-400">{snippet}{(s.distance || s.distance === 0) ? ` • ${s.distance.toFixed(3)}` : ''}</span>
                              </button>
                            );
                          })}
                        </div>
