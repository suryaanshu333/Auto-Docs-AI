...
// Helper: sanitize HTML snippets into readable plain text
function sanitizeSnippet(html) {
  if (!html) return '';
  try {
    // Parse HTML and extract text content (decodes entities)
    const doc = new DOMParser().parseFromString(html, 'text/html');
    let text = doc.body.textContent || '';

    // Normalize non-breaking spaces and multiple whitespace
    text = text.replace(/\u00A0/g, ' ');
    // Insert line breaks for common separators to improve readability
    const separators = ['Job Description:', 'Job Description', 'Job Title', 'Location', 'Duration', 'Responsibilities:', 'Requirements:', 'Qualifications:', 'Skills:', 'Company:'];
    for (const sep of separators) {
      const re = new RegExp(sep, 'gi');
      text = text.replace(re, '\n' + sep + '\n');
    }

    // Convert bullets into newlines
    text = text.replace(/•/g, '\n•');
...
export default function JobsMode({ onBack, docInfo }) {
  ...
  const loadJobs = async () => {
    ...
    try {
      ...
      // If the document isn't a resume, show a friendly message instead of requesting jobs
      if (docInfo?.category && docInfo.category.toLowerCase() !== 'resume') {
        setError('Jobs search works best with a resume. Please upload a resume to find tailored job matches.');
        setLoading(false);
        return;
      }
      ...
      const mappedJobs = (data.jobs || []).map((job) => ({ ...job, snippetFormatted: sanitizeSnippet(job.snippet) }));
      setJobs(mappedJobs);
          ...
          <div className="grid gap-4">
            {jobs.map((job, index) => (
              ...
              >
                ...
                {job.snippet && (
                  <pre className="text-sm text-zinc-400 mb-4 whitespace-pre-wrap">{job.snippetFormatted || sanitizeSnippet(job.snippet)}</pre>
                )}
